                    ...
                    || $varFecha_nac == ""|| $varMail == "")
          {
            ...
            try{
               ...
               or  die("No se conecto a la DB"); #mensaje en caso no conecte
                ...
                $query1 = ' select userName from usuario where userName = "'.$varUsuario.'"
                            ';

                $resp1 = mysqli_query ($conexionDB, $query1);
                $varUserDB = "";
                while ($row = mysqli_fetch_assoc($resp1)){ 
                    $varUserDB = $row['userName'];
                }
                if($varUsuario != $varUserDB){
               ...
               //or die ("<div class='subcontainer'><h4>No se Ingreso el Registro</h4></div>");
                ...
                $query2 = ' insert into cuenta(userName, fechaPago)values(
                            "'.$varUsuario.'",
                            now()
                              )
                            ';
               ...
               echo "<h4 class='bien'>Registro Ingresado</h4>";
                ...
                echo "<div class='error'><h4>Ya existe un usuario con ese nombre</h4></div>";
               }
             }catch(Exception $e){
