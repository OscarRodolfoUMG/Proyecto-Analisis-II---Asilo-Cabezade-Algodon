...
<body>
	...
	<div class="container">
		<h3>Actualizar Entrega Receta</h3>
		...
		<?php 
			...
			$query1 = 'select F.idFicha, C.idConsulta, U.userName, U.nombres, U.apellidos, U.fecha_nac, F.fechaVisita,  C.idConsulta, C.ordenMedicamentos,
				Fa.idFarmacia, Fa.idMedicamento, Fa.cantidadMedicamento, Fa.precioMedicamento, Fa.costoFarmacia
				from ficha_visita F
				inner join usuario U on U.username = F.username
				inner join consulta C on F.idFicha = C.idFicha
				inner join Farmacia Fa on F.idFicha = Fa.idFicha
			...
			while ($row = mysqli_fetch_assoc($resultado)){ 
					...
					$varIdFarmacia= $row['idFarmacia'];
					$varDescripcionMed = $row['idMedicamento'];
					$varCantidadMed = $row['cantidadMedicamento'];
					$varPrecioMed = $row['precioMedicamento'];
					$varMontoMed = $row['costoFarmacia'];
			}

			if(!isset($varMontoMed)){
				$varMontoMed = "0.0";	
			}


			echo '<form action = "upp_farmacia.php" method ="post">
			...
			<h5>ID Farmacia:</h5> 
			<input readonly class="block" type="text" name="idFarmacia" value="'.$varIdFarmacia.'">

			<h5>Ordenes Medicamentos</h5>
			<textarea readonly class="block" name="ordenEx">'.$varOrdenesMed.'</textarea>

			<h5>Descripcion Medicamento</h5>
			<textarea name="descripcionMedicamentos">'.$varDescripcionMed.'</textarea>
			

			<h5>Precio Medicamento</h5> 
			<input id="1" type="text" name="precioMedicamento" value="'.$varPrecioMed.'">

			<h5>Cantidad Medicamento</h5> 
		    <input id="2" type="text" name="cantidadMedicamento" value="'.$varCantidadMed.'">

		    <input  class="btn guardar btn-multi" type="button" id="boton_calc" value="Calcular Monto">

		    <h5>Costo Medicamentos</h5> 
		    <textarea readonly class="text-multi" id="esuma" name="multi">'.$varMontoMed.'</textarea>
			...
			<div class="subcontainer">
				<div class="botones">
					...
					<input class="btn guardar" type="submit" name="" value="Actualizar Entrega">
				</div>
			</div>
